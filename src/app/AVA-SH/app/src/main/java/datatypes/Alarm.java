...
public class Alarm implements ToJSONFile
{
	...
	public Alarm(int hour, int minute, boolean[] days) throws Exception
	{
		this(hour, minute, days, DEFAULT_NAME);
	}
	//generic constructor
	public Alarm(int hour, int minute, boolean[] days, String name) throws Exception
	{
		setName(name);
		setMinute(minute);
		setHour(hour);
	...
	public void setMinute(int minute) throws Exception
	{
		...
		{
			throw new Exception("Minute must be within range of 0 to 59 (inclusive)");
		}
		this.minute = minute;
	}
	public void setHour(int hour) throws Exception
	{
		...
		if (hour < 0 || hour > 23)
		{
			throw new Exception("Hour must be within range of 0 to 23 (inclusive)");
		}
		this.hour = hour;
	}
	public void setDays(boolean[] days) throws Exception
	{
		//check validity
		if (days == null)
		{
			throw new Exception("Days must have a value for each of the 7 days, starting at Monday");
		}
		else
		{
			if(days.length != 7)
